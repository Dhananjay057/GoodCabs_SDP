...
def calender():
    ...
    df = df.withColumn(
    "week_of_year", weekofyear(col("date"))).withColumn("day_of_year", dayofyear(col("date")))

    df = df.withColumn("is_weekend",when(col("day_of_week_num").isin([1, 7]), True).otherwise(False)).withColumn("is_weekday",when(col("day_of_week_num").isin([1, 7]), False).otherwise(True))
